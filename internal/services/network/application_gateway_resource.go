...
func applicationGatewayCustomizeDiff(ctx context.Context, d *pluginsdk.ResourceDiff, _ interface{}) error {
	...
	oldProbes, newProbes := d.GetChange("probe")
	/*
		...
		if oldProbeList := oldProbes.(*schema.Set).List(); len(oldProbeList) == 1 {
			if oldProbeMatch := oldProbeList[0].(map[string]interface{})["match"].([]interface{}); len(oldProbeMatch) > 0 {
				if oldProbeMatchStatusCode := oldProbeMatch[0].(map[string]interface{})["status_code"].([]interface{}); len(oldProbeMatchStatusCode) == 1 && oldProbeMatchStatusCode[0].(string) == "200-399" {
					if newProbeList := newProbes.(*schema.Set).List(); len(newProbeList) == 1 {
						if newProbeMatch := newProbeList[0].(map[string]interface{})["match"].([]interface{}); len(newProbeMatch) == 0 {
							...
							for _, v := range newProbeList {
								...
								updatedProbes.Remove(v)
								updatedProbes.Add(updatedV)
							}
	...
	for _, oldProbe := range oldProbes.(*schema.Set).List() {
		if oldProbeMatch := oldProbe.(map[string]interface{})["match"].([]interface{}); len(oldProbeMatch) > 0 {
			...
			if oldProbeMatchBody == "" && len(oldProbeMatchStatusCodes) == 1 && oldProbeMatchStatusCodes[0].(string) == "200-399" && len(newProbeMatch) == 0 {
				updatedProbe := oldProbe.(map[string]interface{})
				updatedProbe["match"] = oldProbeMatch
				updatedProbes.Remove(oldProbe)
