...
func TestAccApplicationGateway_removeProbesMatch(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_application_gateway", "test")
	r := ApplicationGatewayResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.probesMatch(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		}, /*
...
func (r ApplicationGatewayResource) probesMatch(data acceptance.TestData) string {
	return fmt.Sprintf(`
%s
...
func (r ApplicationGatewayResource) removeProbesMatch(data acceptance.TestData) string {
	return fmt.Sprintf(`
%s
