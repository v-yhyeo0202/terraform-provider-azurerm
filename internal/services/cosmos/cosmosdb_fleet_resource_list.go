...
func (CosmosDbFleetListResource) List(ctx context.Context, request list.ListRequest, stream *list.ListResultsStream, metadata sdk.ResourceMetadata) {
	...
	case !data.ResourceGroupName.IsNull():
		...
		if err != nil {
			sdk.SetResponseErrorDiagnostic(stream, fmt.Sprintf("listing `%s`", fleetResource.ResourceType()), err)
			return
		}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range results {
			...
			id, err := fleets.ParseFleetIDInsensitively(pointer.From(item.Id))
			if err != nil {
				sdk.SetErrorDiagnosticAndPushListResult(result, push, "parsing Fleet ID", err)
				return
			}

			meta := sdk.NewResourceMetaData(metadata.Client, fleetResource)
			meta.SetID(id)

			if err := fleetResource.flatten(meta, id, &item); err != nil {
				sdk.SetErrorDiagnosticAndPushListResult(result, push, fmt.Sprintf("encoding `%s` resource data", fleetResource.ResourceType()), err)
				return
			}
