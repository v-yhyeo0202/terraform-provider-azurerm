...
func TestAccDataProtectionBackupVault_cmkWithSystemAssignedIdentity(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_data_protection_backup_vault", "test")
	r := DataProtectionBackupVaultResource{}
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.cmkWithSystemAssignedIdentity(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (r DataProtectionBackupVaultResource) cmkWithSystemAssignedIdentity(data acceptance.TestData) string {
	template := r.template(data)
	return fmt.Sprintf(`
%s
