...
Manages an Event Grid Namespace Topic ID Association. `azurerm_eventgrid_namespace_topic_id_association` should not be created at the same time with `topic_spaces_configuration.0.route_topic_id` of `azurerm_eventgrid_namespace` resource being set to avoid perpetual change due to different topic ID set via the two methods.
...
```shell
terraform import azurerm_eventgrid_namespace_topic_id_association.example /subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/group1/providers/Microsoft.EventGrid/namespaces/namespace1
```
